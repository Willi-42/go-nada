...
func (l *lossInterval) addLoss(lossGap uint64) {
	...
	l.lossIntervals = append(l.lossIntervals, lossGap)

	// drop oldest int
	if len(l.lossIntervals) >= l.maxsize {
		l.lossIntervals = l.lossIntervals[1:]
	}
}
...
func (l *lossInterval) calcAvgLossInt() float64 {
	...
	if size == 1 {
		return 1 / float64(l.lossIntervals[0])
	}

	// default case
	for i := 0; i < size-1; i++ {
		i_tot0 += float64(l.lossIntervals[i]) * l.weigts[i]
		w_tot += l.weigts[i]
	}

	for i := 1; i < size; i++ {
		i_tot1 += float64(l.lossIntervals[i]) * l.weigts[i-1]
	}
